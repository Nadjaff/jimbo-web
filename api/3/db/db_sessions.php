...
class DbSessions extends DbBase {
	...
	public function session_create($email, $password, $device, $regid) {
		// fetching user by email
		$stmt = $this->conn->prepare("SELECT id, name, email, username, image as userimage, status, created_at, password_hash, fbid, fbtoken FROM users WHERE email = :email OR username = :username");
	...
	public function session_put($api_key, $device, $regid, $fbtoken) {
		...
		//UPDATE users t set t.no_following = t.no_following + 1 WHERE t.id = :id
			$stmt = $this->conn->prepare("UPDATE sessions d set d.regid = COALESCE(:regid, d.regid), d.device = COALESCE(:device, d.device), d.fbtoken = COALESCE(:fbtoken, d.fbtoken) WHERE d.api_key = :api_key");
