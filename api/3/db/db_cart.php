...
class DbCart extends DbBase {
	 ...
	 */
	 public function cart_post($current_user_id, $item_id, $variant_id, $quantity) {
		...
		if (($r = $stmt->fetch(PDO::FETCH_ASSOC)) == NULL){
			...
			$stmt = $this->conn->prepare("INSERT IGNORE INTO cart(user_id,	item_id, variant_id, quantity) VALUES(:user_id, :item_id, :variant_id, :quantity)");
			$stmt->bindParam(":user_id", $current_user_id);
			$stmt->bindParam(":item_id", $item_id);
			$stmt->bindParam(":variant_id", $variant_id);
			...
			$stmt = $this->conn->prepare("UPDATE cart c set c.quantity = :quantity WHERE c.item_id = :item_id AND c.user_id = :user_id AND c.variant_id = :variant_id");
			$stmt->bindParam(":user_id", $current_user_id);
			$stmt->bindParam(":item_id", $item_id);
			$stmt->bindParam(":variant_id", $variant_id);
			$stmt->bindParam(":quantity", $quantity);
	...
	public function cart_update($current_user_id, $cart_id, $variant_id, $quantity) {
		if ($quantity == 0){
			...
			$stmt = $this->conn->prepare("UPDATE cart c set c.quantity = :quantity WHERE c.id = :id AND c.user_id = :user_id AND c.variant_id = :variant_id");
			$stmt->bindParam(":user_id", $current_user_id);
			$stmt->bindParam(":id", $cart_id);
			$stmt->bindParam(":variant_id", $variant_id);
			$stmt->bindParam(":quantity", $quantity);
