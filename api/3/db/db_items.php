...
class DbItems extends DbBase {
	 ...
	 */
	 ...
	 public function items_post_from_url($current_user_id, $title,$price,$description,$quantity,$images,$status,$location_id, $test=null) {
	 	// creating new item
		$db = new DbImages();
	 	// include '../../config.inc';
		...
		if ($status == 1){
			...
			if ($location_id == NULL){
				...
				if ($location["locality"] != NULL){
					$stmt = $this->conn->prepare("INSERT INTO locations(user_id,locality, admin, country, latitude, longitude) VALUES(:user_id, :locality, :admin, :country, :latitude, :longitude)");
...
error_reporting(E_ALL);
			$stmt = $this->conn->prepare("INSERT INTO items(user_id,title,price,currency,description,quantity,status,image,location_id, negotiable, test) VALUES(:user_id, :title, :price, :currency, :description, :quantity, :status, :images, :location_id, :negotiable, :test)");
			$stmt->bindParam(":user_id", $current_user_id);
	...
	public function item_following($current_user_id, $q){
		$stmt = $this->conn->prepare("INSERT IGNORE INTO following_searches(user_id, q) values(:user_id,:q)");
		$stmt->bindParam(":user_id",$current_user_id);
		$stmt->bindParam(":q",$q);	
		if ($stmt->execute()){
	...
	public function item_wanted($current_user_id, $q, $price, $currency, $comment){
		$stmt = $this->conn->prepare("INSERT INTO wanted(user_id, q, price, currency, comment) values(:user_id,:q,:price,:currency,:comment)");
		$stmt->bindParam(":user_id",$current_user_id);
		$stmt->bindParam(":q",$q);
		$stmt->bindParam(":price",$price);
		$stmt->bindParam(":currency",$currency);
	...
	public function item_sales_post($current_user_id, $item_id,$price, $currency, $quantity, $comment){
		...
		$stmt = $this->conn->prepare("INSERT INTO sales(item_id, seller_id, buyer_id, quantity, price, currency, comment) values(:item_id, :seller_id,:buyer_id,:quantity, :price, :currency, :comment)");
