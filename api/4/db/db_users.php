...
class DbUsers extends DbBase {
	...
	public function users_get_all($current_user_id,$q,$newerthan_id,$olderthan_id,$count,$test=0) {
		if ($q == NULL) $q = "";		
		$stmt = $this->limitQuery("SELECT t.username, t.image, t.locality, t.id, t.firstname, t.lastname FROM users t WHERE t.id ### :limitid AND (t.username LIKE CONCAT(:q, '%') OR t.firstname LIKE CONCAT('', :q, '%') OR  t.lastname LIKE CONCAT('% ', :q, '%')) AND t.test = :test ORDER BY t.id DESC LIMIT 0, :count ",$newerthan_id,$olderthan_id,$count);
	 	$stmt->bindParam(':q', $q);		
	...
	public function user_get($current_user_id, $user_id,$username, $newerthan_id,$olderthan_id,$count, $test) {
		...
		$qry = "SELECT lo.latitude,lo.longitude,lo.address, t.id, t.username,t.gender, t.firstname, t.lastname, t.stars, t.sales, t.bio, t.website, t.location_description, t.no_products as num_products, t.no_followers as num_followers, t.no_following as num_following, t.locality, t.image as userimage";
		if ($user_id == $current_user_id){
			//$qry .= ", t.phone, t.address, t.gender, t.email";
			// TODO: these details should be returned if it's a user querying himself.
		}
		$qry .=" from users t ";
		$qry .=" left  join locations lo on lo.id=t.location_id ";
		$qry .= "WHERE t.id = :id";
		...
		$stmt = $this->limitQuery("SELECT t.image as userimage, t.id, t.title,  " . $this->getpricet	. ",  t.created_at from items t  LEFT JOIN users AS ut ON ut.id = t.user_id " . $this->getpricetablest	. " WHERE t.id ### :limitid AND t.test = :test AND t.status < 5 AND t.user_id = :id ORDER BY t.id DESC LIMIT 0, :count",$newerthan_id, $olderthan_id,$count);
		$stmt->bindParam(":id",$user_id);		
		//$stmt->bindParam(":uid",$current_user_id);		
		...
		if (count($response["items"]) == 0 && $newerthan_id != 0){
			$stmt = $this->limitQuery("SELECT t.image as userimage, " . $this->getpricet	. ", t.id, t.title, t.created_at from items t 
	...
	public function user_get_initials($user_id) {
		$err = 0;
		$response = array();
		$stmt = $this->conn->prepare("SELECT t.firstname, t.lastname, t.col FROM users t WHERE t.id = :uid");
		$stmt->bindParam(":uid", $user_id);
	...
	public function user_get_settings($current_user_id, $user_id) {
		...
		$stmt->bindParam(":uid", $current_user_id);$this->singleExecute($stmt);
	...
	public function user_update_all(){
		...
		foreach ($a as $l){
			...
			if (substr($l["image"],0,7) == "default"){
				...
				$stmt2 = $this->conn->prepare("UPDATE users t SET t.image = :image,updated_at=:updated_at WHERE t.image = :oldimage AND t.id = :id");
				$stmt2->bindParam(":id", $l["id"]);
				$stmt2->bindParam(":image", $image);
				$stmt2->bindParam(":oldimage", $l["image"]);
			   $stmt2->bindParam(":updated_at", date('Y-m-d H:i:s'));
				echo $stmt2->execute();
			}
	...
	public function users_post($firstname, $lastname, $username, $email, $password, $lat, $long, $locality, $country,$admin, $phone, $fbid, $fbtoken, $image, $mobile_code, $test=0) {
		...
		if (!$this->isEmailExists($email)) {
			if (!$this->isUserExists($username)) {
				...
				if ($image == "default.jpg"){
					$l1 = substr($firstname,0,1);
					$l2 = substr($lastname,0,1);
				...
				// insert query
				$stmt = $this->conn->prepare("INSERT INTO users(firstname, lastname, username, email, password_hash, phone, fbid, fbtoken, image, api_key, status, mobile_code, col, test,created_at,updated_at) values(:firstname, :lastname, :username, :email, :password_hash, :phone, :fbid, :fbtoken, :image, :api_key, :status, :mobile_code, :col, :test,:created_at,:updated_at)");
				...
				$stmt->bindParam(":created_at", $cre_dt);
			    $stmt->bindParam(":updated_at", $cre_dt);
				$bool = $stmt->execute();
				...
				if ($bool) {
					...
					$stmt = $this->conn->prepare("INSERT INTO user_settings(id, currency,created_at,updated_at) values(:id, :currency,:created_at,:updated_at)");
					$curr = "AUD";					
					...
					$stmt = $this->conn->prepare("INSERT INTO locations(name, latitude,longitude,country,
					locality,admin,user_id,
					created_at,updated_at) values(:name, :latitude,:longitude,:country,:locality,:admin,:user_id,
			        ...
			        $stmt->bindParam(":updated_at", $cre_dt);
					$stmt->execute();
					$stmt = $this->conn->prepare("UPDATE users  u set u.location_id=:location_id where id=:user_id");
	           	   	$stmt->bindParam(":location_id",$this->conn->lastInsertId());
					$stmt->bindParam(":user_id",$response["id"] );
				   $stmt->execute();
					return $response;
				} else {
	...
	public function user_follow($current_user_id, $follow_id, $val){
		...
		if ($val == 1){
			if ($num_rows == 0){
				$stmt = $this->conn->prepare("UPDATE users t set t.no_following = t.no_following + 1,updated_at= :updated_at  WHERE t.id = :id");
				$stmt->bindParam(":id", $current_user_id);
				$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
				$stmt->execute();
				
				$stmt = $this->conn->prepare("UPDATE users t set t.no_followers = t.no_followers + 1,updated_at= :updated_at WHERE t.id = :id");
				$stmt->bindParam(":id", $follow_id);
				$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
				$stmt->execute();
				
				$stmt = $this->conn->prepare("INSERT INTO following(user_id, following_id,created_at,updated_at) values(:id,:fid,:created_at,:updated_at)");
			...
			if ($num_rows == 1){
				$stmt = $this->conn->prepare("UPDATE users t set t.no_following = t.no_following - 1,t.updated_at=:updated_at WHERE t.id = :id");
				$stmt->bindParam(":id", $current_user_id);
				$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
				$stmt->execute();
				;
				$stmt = $this->conn->prepare("UPDATE users t set t.no_followers = t.no_followers - 1 ,t.updated_at=:updated_at  WHERE t.id = :id");
				$stmt->bindParam(":id", $follow_id);
				$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
				$stmt->execute();
				;
	...
	public function user_clear_fb($current_user_id){
		$blank = "";
		$zero = 0;
		$stmt = $this->conn->prepare("UPDATE users t set t.fbid = :zero, t.fbtoken = :blank,	updated_at=:updated_at WHERE t.id = :id");
		$stmt->bindParam(":id", $current_user_id);
	...
	public function user_put($current_user_id, $user_id, $username, $firstname, $lastname, $bio, $website, $phone, $email, $location_description, $gender, $img) {
		...
		if (($username == NULL || $username == "") && $img != ""){
			...
			if (substr($img,0,7) == "default"){
				...
				$col = $this->mix_color();
				$image = "default_" . $col . "_" . $l1 . $l2 . ".jpg";
				$stmt = $this->conn->prepare("UPDATE users t SET t.image = :image, t.updated_at = :updated_at WHERE t.id = :id");
				$stmt->bindParam(":id", $user_id);
				$stmt->bindParam(":image", $image);
				$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
				$img = $image;
		...
		$stmt = $this->conn->prepare("UPDATE users t set t.updated_at=:updated_at, t.username = :username, t.firstname = :firstname, t.lastname = :lastname, $set_bio $set_phone $set_location_description $set_gender $set_website t.email = :email WHERE t.id = :id");
		$stmt->bindParam(":username", $username);
		$stmt->bindParam(":firstname", $firstname);
		$stmt->bindParam(":lastname", $lastname);
		$stmt->bindParam(":email", $email);
	...
	public function user_put_password($current_user_id, $user_id, $password, $newpassword) {
		...
		if ($password != "" && $newpassword != ""){
		 ...
		 $stmt = $this->conn->prepare("UPDATE users t set t.password_hash = :password_hash, updated_at=:updated_at WHERE t.id = :id");
			$stmt->bindParam(":password_hash", $password_hash);
			$stmt->bindParam(":id", $user_id);
			$stmt->bindParam(":updated_at", date('Y-m-d H:i:s'));
			if (!$stmt->execute()) {
	...
	public function user_deactivate($current_user_id, $user_id) {
	...
	public function createToken($username) {
		...
		if ($res != NULL){
			...
			$stmt = $this->conn->prepare("INSERT INTO tokens(user_id, token,created_at,updated_at) values(:user_id, :token,:created_at,:updated_at)");
