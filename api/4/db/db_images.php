...
class DbImages extends DbBase {
	...
	public function createImage($current_user_id, $image) {
		$stmt = $this->conn->prepare("INSERT INTO images(user_id,image,created_at,updated_at) VALUES(:id, :image,:created_at,:updated_at)");
		$stmt->bindParam(":id",$current_user_id);
		$stmt->bindParam(":image",$image);
		$cur= date('Y-m-d H:i:s');
        $stmt->bindParam(":created_at", $cur);
	    $stmt->bindParam(":updated_at", $cur);		
		$result = $stmt->execute();	
		if ($result) {
			// item row created
