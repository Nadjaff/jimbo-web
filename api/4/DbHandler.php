...
class DbHandler {
	...
	public function image_get($current_user_id,$image_id) {
		$stmt = $this->conn->prepare("SELECT image, item_id FROM images WHERE id = :id AND user_id = :user_id");
    ...
    public function deleteImage($current_user_id, $item_id) {
        $stmt = $this->conn->prepare("DELETE t FROM images t WHERE t.id = :id AND t.user_id = :user_id");
