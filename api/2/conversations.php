...
$app->post('/conversations', 'authenticate', function() use ($app) {
	...
	if (($user_id == NULL || $user_id == "") && ($item_id == NULL && $item_id == "")){
        verifyRequiredParams(array('item_id','user_id'));
	}
	$response = $db->conversation_post($current_user_id,NULL,$item_id,$user_id,$message,$type);
	if ($response != NULL) {
		$response["error"] = 0;
...
$app->post('/conversations/:id', 'authenticate', function($conversation_id) use ($app) {
	...
	$message = $app->request->post('message');
	$type = $app->request->post('type');
	$response = $db->conversation_post($current_user_id,$conversation_id,NULL,NULL,$message,$type);
	if ($response != NULL) {
		echoResponse(201, $response);
	} else {
		$response = array();
