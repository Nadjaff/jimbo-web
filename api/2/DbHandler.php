...
class DbHandler {
     ...
     */
    public function user_get($current_user_id, $user_id, $newerthan_id, $olderthan_id, $count) {
		if ($count == 0){
			$count = 20;
		}
		if ($count > 100){
		...
		if ($stmt->execute()) {
			...
			if ($stmt->execute()) {
				...
				$selectqry = "SELECT t.price, t.image, t.id from items t WHERE t.user_id = ?";
				$searchqry = "ORDER BY t.id DESC LIMIT 0, ?";
				if ($newerthan_id != 0){
					$stmt = $this->conn->prepare("$selectqry AND t.id > ? $searchqry");
					$stmt->bind_param("iii",$user_id, $newerthan_id,$count);
				}else if ($olderthan_id != 0){
					$stmt = $this->conn->prepare("$selectqry AND t.id < ? $searchqry");
					$stmt->bind_param("iii",$user_id, $olderthan_id,$count);
				}else{
					$stmt = $this->conn->prepare("$selectqry $searchqry");
